...
model = CNN().to(device)
model.load_state_dict(torch.load("TuberChestPrediction/tuber_model.pth", map_location=device))
model.eval()
...
def get_sample_images(class_name, num_samples=3):
    ...
    class_folder = os.path.join("TuberChestPrediction", class_name)
...
if __name__ == "__main__":
    image_path = "TuberChestPrediction/testtuber.png"
    ...
    result, confidence = predict_and_visualize(
        image_path, 
        save_path="TuberChestPrediction/prediction_visualization.png",
        show=True
    )
    print(f"Visualized Prediction: {result} (Confidence: {confidence:.2f})")
    print("Visualization saved to 'TuberChestPrediction/prediction_visualization.png'")
